...
const HistoryContext = createContext<{
  ...
  state: defaultState,
  updateState: () => {
    throw new Error("HistoryContext not provided")
  },
})
...
export function useHistoryStateLense<P extends keyof HistoryState>(
  ...
  try {
    if (validate && !validate(value)) value = defaultState[prop]
  } catch (e) {
...
export function HistoryContextProvider({ children }: Props) {
  ...
  const context = useMemo(
    () => ({
      state,
      updateState: (action: UpdateStateAction<HistoryState>) => {
        updateState(action)
      },
    }),
    [state]
